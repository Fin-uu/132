...
# ... (infer 函數和 check_channel 函數, 保持不變)
# ... (eval_label_pred 函數, 保持不變)
...
# UPDATED: Replaced AddChannel() with EnsureChannelFirst()
# 這是最穩定的版本，使用 torch.unsqueeze 來確保 5D 輸出
def check_channel(inp):
    len_inp_shape = len(inp.shape)
    
    # 我們的目標是確保形狀是 5D: (B, C, D, H, W)
    
    if len_inp_shape == 4:
        # 如果是 4D (通常是: B, D, H, W)，表示缺少 C 維度 (Channel)
        # 在 dim=1 (Channel 的位置) 插入維度 1
        inp = torch.unsqueeze(inp, dim=1) # 結果: (B, 1, D, H, W) -> 5D
        
    elif len_inp_shape == 3:
        # 如果是 3D (通常是: D, H, W)，表示缺少 C 和 B 維度
        # 1. 在 dim=0 插入 C 維度: (1, D, H, W)
        inp = torch.unsqueeze(inp, dim=0)
        # 2. 在 dim=0 插入 B 維度: (1, 1, D, H, W)
        inp = torch.unsqueeze(inp, dim=0) 
...
def eval_label_pred(data, cls_num, device):
    ...
    # 這是您 eval_label_pred 函數的後半部分，請確保它被正確地放在文件中：
